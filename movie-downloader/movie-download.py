...
制作花絮: 此 skill 由 Waylon 通过 OpenClaw 全自动制作
         （和龙虾来回拉扯了好久），希望大家用的开心！
"""
...
def check_dependencies():
    ...
    try:
        ...
        # 检查 chromium
        chromium_path = Path.home() / ".cache/ms-playwright/chromium-*/chrome-mac/Chromium.app"
        if not list(Path.home().glob(".cache/ms-playwright/chromium-*")):
...
async def search_movie(query: str):
    ...
    search_url = f"https://rargb.to/search/?search={encoded_query}"
    
    print_color(Colors.BLUE, f"🔗 打开: {search_url}")
    print()
                ...
                try:
                    ...
                    if len(cells) >= 6:
                        ...
                        if name_link:
                            ...
                            results.append({
                                "name": name.strip(),
                                "detail_url": "https://rargb.to" + detail_url if detail_url.startswith("/") else detail_url,
                                "size": size.strip(),
